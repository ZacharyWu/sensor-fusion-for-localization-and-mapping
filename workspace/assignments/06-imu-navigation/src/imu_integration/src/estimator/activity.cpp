...
bool Activity::UpdatePose(void) {
    if (!initialized_) {
        ...
        // get angular deltas: GetAngularDelta
...
bool Activity::GetAngularDelta(
    ...
    // angular_delta = 0.5*delta_t*(angular_vel_curr + angular_vel_prev);  // mid
    angular_delta = delta_t * angular_vel_prev; // Euler
...
bool Activity::GetVelocityDelta(
    ...
    // velocity_delta = 0.5*delta_t*(linear_acc_curr + linear_acc_prev);   // mid
    velocity_delta = delta_t * linear_acc_prev;   // Euler
