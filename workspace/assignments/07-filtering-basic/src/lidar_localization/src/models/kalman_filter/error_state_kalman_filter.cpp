...
bool ErrorStateKalmanFilter::Update(const IMUData &imu_data) {
  //
  // TODO: understand ESKF update workflow: // Predication
  //
  // update IMU buff:
...
bool ErrorStateKalmanFilter::GetVelocityDelta(
  ...
  linear_acc_mid = 0.5 * (linear_acc_curr + linear_acc_prev) - accl_bias_; // updated from navigation to body 
 ...
 * @param  w_b, gyro measurement in body frame
 * @return void
 */
void ErrorStateKalmanFilter::SetProcessEquation(const Eigen::Matrix3d &C_nb,
  ...
  // a. set process equation for delta vel: // Ft in PPT
  F_.setZero();
  ...
  // b. set process equation for delta ori: // Bt in PPT
  B_.setZero();
  B_.block<3, 3>(kIndexErrorVel, kIndexNoiseAccel) = C_nb; // (3,0)
  B_.block<3, 3>(kIndexErrorOri, kIndexNoiseGyro) = Eigen::Matrix3d::Identity();     // (6,3)
  B_.block<3, 3>(kIndexErrorAccel, kIndexNoiseBiasAccel) = Eigen::Matrix3d::Identity();   // (9,6)
  B_.block<3, 3>(kIndexErrorGyro, kIndexNoiseBiasGyro) = Eigen::Matrix3d::Identity();  // (12,9)
...
void ErrorStateKalmanFilter::UpdateProcessEquation(
  ...
  // Eigen::Vector3d f_n = linear_acc_mid + g_; //navigation frame
  Eigen::Vector3d f_b = linear_acc_mid; //body frame
...
void ErrorStateKalmanFilter::UpdateErrorEstimation(
  ...
  MatrixB B = MatrixB::Zero();  // Bk-1 in PPT
  B.block<3, 3>(kIndexErrorVel, kIndexNoiseAccel) = T * B_.block<3, 3>(kIndexErrorVel, kIndexNoiseAccel);   // (3,0)
  B.block<3, 3>(kIndexErrorOri, kIndexNoiseGyro) = T * B_.block<3, 3>(kIndexErrorOri, kIndexNoiseGyro);     // (6,3)
  B.block<3, 3>(kIndexErrorAccel, kIndexNoiseBiasAccel) = std::sqrt(T) * B_.block<3, 3>(kIndexErrorAccel, kIndexNoiseBiasAccel);  // (9,6)
  B.block<3, 3>(kIndexErrorGyro, kIndexNoiseBiasGyro) = std::sqrt(T) * B_.block<3, 3>(kIndexErrorGyro, kIndexNoiseBiasGyro);  // (12,9)

  // TODO: perform Kalman prediction
  P_ = F * P_ * F.transpose() + B * Q_ * B.transpose(); // F * X_ + B * n   // Equ(1)
...
void ErrorStateKalmanFilter::CorrectErrorEstimationPose(
    ...
    Eigen::MatrixXd &K) {
  //
  // TODO: set measurement: T = [R, p]
  //
  Eigen::Vector3d dp = pose_.block<3, 1>(0, 3) - T_nb.block<3, 1>(0, 3); //delta_p = ~p (predict) - p (observe)
